...
func (h *eventHandler) OnConnect(c *centrifuge.Client, e centrifuge.ConnectEvent) {
	notifier.Info(nil, fmt.Sprintf("Connected to %s", h.url))
}

func (h *eventHandler) OnError(c *centrifuge.Client, e centrifuge.ErrorEvent) {
	notifier.Error(nil, fmt.Sprintf("Error: %s", e.Message))
}

func (h *eventHandler) OnDisconnect(c *centrifuge.Client, e centrifuge.DisconnectEvent) {
	notifier.Error(nil, fmt.Sprintf("Disconnected from %s", h.url))
	h.stop = true
}
func (h *eventHandler) OnSubscribeSuccess(sub *centrifuge.Subscription, e centrifuge.SubscribeSuccessEvent) {
	notifier.Info(nil, fmt.Sprintf("Subscribed to channel %s", sub.Channel()))
}

func (h *eventHandler) OnSubscribeError(sub *centrifuge.Subscription, e centrifuge.SubscribeErrorEvent) {
	notifier.Error(nil, fmt.Sprintf("Failed to subscribe to channel %s, error: %s", sub.Channel(), e.Error))
}

func (h *eventHandler) OnUnsubscribe(sub *centrifuge.Subscription, e centrifuge.UnsubscribeEvent) {
	notifier.Info(nil, fmt.Sprintf("Unsubscribed from channel %s", sub.Channel()))
}
...
func (h *eventHandler) OnPublish(sub *centrifuge.Subscription, e centrifuge.PublishEvent) {
	...
	if err != nil {
		notifier.Error(nil, fmt.Sprintf("Error: %s", err))
		return
	}
	if response.Action != "saved" {
		notifier.Warn(nil, fmt.Sprintf("Unknown action received: %s", string(e.Data)))
	}
	...
	notifier.Info(nil, fmt.Sprintf("%s: %s updated from dashboard %s", now, h.filename, response.UID))
}
...
func (h *eventHandler) WaitForStop() {
	for {
		...
		if h.stop {
			notifier.Warn(nil, "Stopping.")
			os.Exit(1)
		}
	}
}
func watchDashboard(UID, filename string) error {
	wsURL, token, err := getWSGrafanaURL("api/live/ws")
	fmt.Println(wsURL, token)
	if err != nil {
		return err
	}
